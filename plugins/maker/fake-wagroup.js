...
export default {
   ...
   async run(m, {
      ...
      try {
         ...
         const [name = m.pushName, members = `${randomInteger(1, 1024)}`, desc = 'Alxena Community', author = m.pushName, date = '27/05/26 11:11'] = text.split('|')
         if (!text)
            return m.reply(`👉🏻 *Example*: ${isPrefix + command} Alxena | 1024 | Alxena Community | @sairidev | 27/05/26 11:11`)
         m.react('🕒')
         let profilePicture
         if (mimetype)
