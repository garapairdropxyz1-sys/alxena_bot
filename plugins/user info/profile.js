import { areJidsSameUser } from '@sairidev/baileys-new'
...
export default {
   ...
   async run (m, {
      ...
      const printUserInfo = frame('USER INFO', [
         `Name    : ${userData.name}`,
         `Limit   : ${isPartner ? 'Unlimited' : userData.limit}`,
         `Energy  : ${isPartner ? 'Unlimited' : userData.energy}`,
      ])
      const printUserStats = frame('USER STATS', [
         `Commands: ${userData.commandUsage}x`,
         `Warning : ${warningPoint ?? '-'} point`,
         `Partner : ${isPartner ? 'yes' : 'no'}`,
         `Premium : ${userData.premiumExpiry > 0 ? 'yes' : 'no'}`,
