import { isJidNewsletter } from '@sairidev/baileys-new'
...
const wrapScript = () => {
   const filePath = join(temporaryFolder, `Alxena-${createFileName()}.zip`)
...
const updateScript = async () => {
   const zipPath = await persistToFile('https://github.com/sairidev')
...
export default {
   ...
   async run (m, {
      ...
      if (
         ...
         if (!text)
            return m.reply(`👉🏻 *Example*: ${isPrefix + command} You're Alxena AI, developed and maintained by Starfall Co. Ltd.`)
         ...
         if (!text)
            return m.reply(`👉🏻 *Example*: ${isPrefix + command} Alxena`)
         if (text.length > 25)
