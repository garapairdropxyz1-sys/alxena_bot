 ...
 * https://github.com/sairidev/denia
 *
 * All rights reserved.
 *
...
import { delay, DONATE_URL, isJidGroup, prepareWAMessageMedia, S_WHATSAPP_NET, WA_DEFAULT_EPHEMERAL } from '@sairidev/baileys-new'
...
export const createSticker = async (media, options = {}) => {
   ...
   return writeExif(media, {
      'sticker-pack-id': 'sairidev',
      'sticker-pack-name': options.stickerPackName ?? stickerPackName,
      'sticker-pack-publisher': options.stickerPackPublisher,
      'android-app-store-link': 'https://github.com/sairidev',
      'ios-app-store-link': 'https://github.com/sairidev',
      emojis: ['✨'],
...
export const frame = (title, lines = []) => {
   const label = `[ ${title} ]`
...
// ─── ANSI color helpers for messageLogger ───────────────────────────────────
const _c = {
   reset:   '\x1b[0m',
   bold:    '\x1b[1m',
   dim:     '\x1b[2m',
...
const _typeColor = (type = '') => {
   const t = type.toLowerCase()
   if (t.includes('image'))    return _c.green
   if (t.includes('video'))    return _c.magenta
   if (t.includes('audio') || t.includes('sticker')) return _c.yellow
   if (t.includes('document')) return _c.blue
   if (t.includes('protocol')) return _c.gray
   if (t.includes('reaction')) return _c.cyan
   if (t.includes('extended') || t.includes('text')) return _c.white
   return _c.gray
}

const _box = (lines, color = '') => {
   const clean = lines.map(l => l.replace(/\x1b\[[0-9;]*m/g, ''))
   const width = Math.max(...clean.map(l => l.length)) + 2
   const top    = `${color}┌${ '─'.repeat(width) }┐${_c.reset}`
   const bottom = `${color}└${ '─'.repeat(width) }┘${_c.reset}`
   const mid    = lines.map((l, i) => {
      const pad = width - 2 - clean[i].length
      return `${color}│${_c.reset} ${l}${' '.repeat(pad)} ${color}│${_c.reset}`
   })
...
export const messageLogger = (message) => {
   ...
   const name     = message.pushName || message.verifiedBizName || 'unknown'
   ...
   const tc = _typeColor(type)

   const chatVal = isStatus
      ? `${_c.yellow}${_c.bold}status@broadcast${_c.reset}`
      : isGroup
         ? `${_c.magenta}${chat}${_c.reset}`
         : `${_c.blue}${chat}${_c.reset}`

   const lines = [
      `${_c.gray}${time}${_c.reset}  ${_c.bgBlue}${_c.white}${_c.bold} IN ${_c.reset}  ${tc}${_c.bold}${type.padEnd(22)}${_c.reset}${_c.cyan}${_c.bold}${sender}${_c.reset} ${_c.gray}(${name})${_c.reset}`,
      `${_c.gray}CHAT${_c.reset}  ${chatVal}`,
      ...(body ? [`${_c.gray}MSG ${_c.reset}  ${_c.white}${body}${_c.reset}`] : [])
   ]
